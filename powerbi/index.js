...
const bucket = 'smas-prod-data-export-for-pipeline'
const yesterday = moment().subtract(5, 'days').format('YYYY-MM-DD')
...
export default () =>
  new Promise(async (resolve, reject) => {
    try {
      ...
      await sendCronConfirmationEmail('PowerBI Data Service', results)
      console.log(results)
      resolve(results)
    } catch (error) {
      await sendCronConfirmationEmail('PowerBI Data Service Error', error)
      console.log(error)
      reject(error)
    }
  })
